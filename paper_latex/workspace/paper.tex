...
\begin{figure*}[!t]
  ...
  \caption{Visualization of competing epidemic spreading fronts for the sentence ``Every professor who advises some student wins a prize.''
  The `every' front (orange) propagates freely across nsubj and dobj edges but is blocked at the relcl boundary (dashed edge,
  $\beta_{\text{relcl}} = 0.06$). The `some' front (purple) is contained within the relative clause. Token `advises' is contested
  (both fronts arrive with nearly equal intensity), triggering dual-reading output.}
...
\begin{figure*}[!t]
  ...
  \caption{Detailed view of the neuro-symbolic interface. The LLM returns a structured JSON with predicate names, arities, and entity
  variables but with scope left as null. The epidemic scope model fills in the quantifier nesting and variable binding to
  produce the final FOL formula. For contested sentences, two formulas are assembled simultaneously.}
...
\begin{figure*}[!t]
  ...
  \caption{FOL exact-match accuracy and z3 logical equivalence accuracy on the FOLIO test set for all systems. EpiScope outperforms
  both the direct-LLM baseline (B1) and GNN typed-message-passing baseline (B3) by ${\geq}10$ percentage points exact-match
  ($p < 0.001$, bootstrap resampling). Error bars show 95\% confidence intervals from bootstrap resampling.}
...
\begin{figure*}[!t]
  ...
  \caption{Learned transmission rates for the 12 most frequent UD dependency relation types after L-BFGS-B optimization on the FOLIO
  training set. Core grammatical relations (nsubj, dobj) learn high $\beta$; modification edges (amod, advmod) learn intermediate
  $\beta$; clause-boundary edges (relcl, advcl, ccomp) converge to near-zero, confirming the scope-island hypothesis. Dashed
